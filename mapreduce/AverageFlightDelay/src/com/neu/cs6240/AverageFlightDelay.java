...
public class AverageFlightDelay {
	...
	public static class AverageFlighDelayMapper {
		...
		private boolean isValidEntry(String[] record) {
			...
			if (record[0].isEmpty() || record[2].isEmpty() || record[5].isEmpty() || record[11].isEmpty()
					|| record[17].isEmpty() || record[24].isEmpty() || record[35].isEmpty() || record[37].isEmpty()
		 ...
		 * 
		 * @param record
		 *            : array of string from input record
	 ...
	 */
	public static class AverageFlightDelayReducer extends Reducer<Text, Text, Text, Text> {
			...
			for (String originatedAtORD : originatedAtORDList) {
				for (String arrivedAtJFK : arrivedAtJFKList) {
					...
					if (isTwoLeggedFligh(originatedAtORDData, arrivedAtJFKData)) {
						float delay = Float.parseFloat(originatedAtORDData[3]) + Float.parseFloat(arrivedAtJFKData[3]);
		...
		/**
		 * Whether given two flight data is valid entry for two legged flight starting
		 * at ORD and arriving at JFK?
		 * 
		 * @param originatedAtORDData
		 *            array of string data of flight originated at ORD
		 * @param arrivedAtJFKData
		 *            array of string data of flight arriving at JFK
		 * @return true iff this connection if valid two legged flight with valid time
		 *         false otherwise
		 */
		private boolean isTwoLeggedFligh(String[] originatedAtORDData, String[] arrivedAtJFKData) {
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			...
			context.write(new Text("--" + context.getCounter(HadoopCounter.TotalDelay).getValue()),
					new Texttext.getCounter(HadoopCounter.TotalFlights).getValue()));
		}
	}
