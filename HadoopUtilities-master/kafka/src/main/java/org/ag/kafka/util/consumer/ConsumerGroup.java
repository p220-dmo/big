...
public class ConsumerGroup<K, V> {
	...
	public ConsumerGroup(ConsumerConfig consCfg, RecordHandler<K, V> handler) {
		...
		IntStream.range(0, Integer.valueOf(this.conf.getProps().get("consumer.number").toString()))
				.forEach(i -> consumers.add(new CustomConsumer(this.conf, this.handler)));
	}

	public void start() {
		consumerThreads = new ArrayList<>();
		consumers.stream().forEach(cust -> {
			Thread th = new Thread(cust);
			th.start();
